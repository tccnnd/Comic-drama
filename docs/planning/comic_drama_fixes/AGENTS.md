...
strategic priority is to complete the director-interpretation layer (v0.5.0)
and move toward shot-level video rendering (next spec), while preserving a
...
```powershell
python -m py_compile scripts\run_workflow.py backend\project_runtime.py backend\app.py video_providers.py scripts\video_provider_adapters.py backend\video_generation.py backend\scene_renderer.py
...
The project is currently at **v0.5.0** (director-interpretation-mainline).
Delivered feature lines:

```text
v0.2.0: video-provider-mainline         — real video as primary renderer
v0.3.0: global-consistency-governance  — five-dimension continuity + ledger
v0.4.0: director-review-console        — triage, batch rerender, review units
v0.5.0: director-interpretation        — director_plan + visual_prototype + visual_content (in progress)
```

The next planned feature line is:

```text
next: shot-level-video-rendering — render each shot_plan.shots[] through the
      video provider, assemble shot clips into a scene clip, persist per-shot
      provenance. Draft spec in .kiro/specs/shot-level-video-rendering/.
```
